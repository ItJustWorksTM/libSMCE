...
void exit_sketch(int code) { std::cerr << "Error code:  " << code << std::endl; }
...
void print_menu() {
    std::cout << "SMCE Client menu:\n"
              << "-h -> See menu\n"
              << "-p -> Pause or resume the board\n"
              << "-rc -> Recompile the sketch\n"
              << "-m <message> -> Send message to board through uart (serial)\n"
              << "-mt -> disable uart prints in console\n"
              << "-wa <pin> <value> -> Set a specific value on a analog pin\n"
              << "-wd <pin> <value> -> Set a specific value on a digital pin, value should be 0 or 1\n"
              << "-ra <pin> -> Read the value on a analog pin\n"
              << "-rd <pin> -> Read the value on a digital pin\n"
...
int compile_sketch(smce::Sketch& sketch, smce::Toolchain& toolchain, smce::Board& board, std::string arduino_root_dir) {
    ...
    smce::BoardConfig board_conf{
        ...
        .gpio_drivers = {smce::BoardConfig::GpioDrivers{10, smce::BoardConfig::GpioDrivers::DigitalDriver{true, true},
                                                        smce::BoardConfig::GpioDrivers::AnalogDriver{true, true}},
                         smce::BoardConfig::GpioDrivers{1, smce::BoardConfig::GpioDrivers::DigitalDriver{false, true},
...
int main(int argc, char** argv) {
    ...
    auto cli =
        lyra::help(show_help) | lyra::opt(fqbn, "fqbn")["--fpqn"]["-f"]("Fully qualified board name") |
        lyra::opt(path_to_sketch, "path-to-sketch")["--path"]["-p"]("The path to the sketch") |
        lyra::opt(arduino_root_dir,
                  "Ardunio home folder")["--dir"]["-d"]("The absolute path to the desired location of Arduino root") |
        lyra::opt(smce_resource_dir,
                  "Alternative path to SMCE_RESOURCE") // Makes it possible to change path to SMCE_RESOURCES at start.
            ["--SMCE"]["-s"]("Tha alternative path to SMCE_RESOURCES for runtime, should be absolute.") |
        lyra::opt(file_write, "file_write")["--file"]["-u"](
            "Set to true if uart should write to file (created in the set Arduino root folder)");
    ...
    // clang-format off
    smce::Sketch sketch{path_to_sketch, {.fqbn = fqbn, .legacy_preproc_libs = {{"WiFi"}, {"MQTT"}}}};
    // // clang-format on
    ...
    std::cout << "Messages received on uart from arduino is shown as " << termcolor::red << "red" << termcolor::reset
              << " text." << std::endl;
    // Print command menu
    print_menu();
    ...
    while (true) {
        ...
        if (input == "-h") { // help menu
            ...
            if (board.status() != smce::Board::Status::suspended) {
                if (board.suspend())
                    std::cout << "Board paused" << std::endl;
                else
                    std::cout << "Board could not be paused" << std::endl;
            } else if (board.status() == smce::Board::Status::suspended) {
                board.resume();
            ...
            const int pin = stoi(input.substr(3, 5));
            const uint16_t value = stoi(input.substr(5));
            auto apin = board_view.pins[pin].analog();
            if (!apin.exists()) {
                std::cout << "Pin does not exist!" << std::endl;
            }else if (!apin.can_write()) {
            ...
            const int index_pin = stoi(input.substr(3));

...
for f in $(find "${base_dir}" -type d \( -path "${base_dir}/build" -o -path "${base_dir}/cmake-build*" -o -path "${base_dir}/boost" -o -path "${base_dir}/boost_root" \) -prune -o \( -name 'CMakeLists.txt' -o -name '*.cmake' \)); do
  echo 'Scanning' $f
...
sed -e "s#${base_dir}/build/test/test_dir/manifests#${base_dir}/test/manifests#g" \
    ...
    CMakeTrace/*.json \
| jq -c "select(has(\"version\") | not) | select(.file | startswith(\"${base_dir}/build\") | not) | select(.file | startswith(\"${base_dir}/boost\") | not) | select(.file | startswith(\"${base_dir}\")) | {\"file\": .file, \"line\": .line}" \
